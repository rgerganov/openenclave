...
static int cert_verify_callback(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags) 
{
	...
	if (result != OE_OK)
	{
		printf("oe_verify_tls_cert failed with result = %s\n", oe_result_str(result));	
		goto exit;
	}
	ret = 0;
...
int setup_tls_server(char* server_port)
{
	...
	printf("Setup the listening TCP socket on SERVER_IP= [%s] server_port = [%s]\n", SERVER_IP, server_port);
	...
	printf("mbedtls_net_bind returned successfully. (listen_fd = %d)\n", listen_fd.fd);
...
waiting_for_connection_request:
	...
	printf("mbedtls_net_accept returned successfully.(listen_fd = %d) (client_fd = %d) \n", listen_fd.fd, client_fd.fd);
	...
	do
	{
		...
		{
			printf("ERROR: expected reading %d bytes but only got %d bytes\n", CLIENT_REQUEST_PAYLOAD_SIZE, len);
