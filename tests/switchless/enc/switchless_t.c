...
oe_result_t handle_synchronous_switchless_ecall(tc_queue_node* p_queue_node)
{
    ...
    // Signal the host.
    // oe_host_printf("    </handle_synchronous_switchless_ecall>\n");
    atomic_uint32_store(&(p_queue_node->data.sync.lock), 1);
    ...
    size_t* output_bytes_written)
{
    ...
    size_t count = 0;
    while (TC_RUNNING == tc_get_state(pargs_in->ptc) &&
           pargs_in->ptc->count_limit > count++)
    {
        tc_queue_node* p_queue_node = tc_pop_enc_queue(pargs_in->ptc);
        if (p_queue_node)
        {
            count = 0;
    ...
    tc_set_state(pargs_in->ptc, TC_STOPPED);
